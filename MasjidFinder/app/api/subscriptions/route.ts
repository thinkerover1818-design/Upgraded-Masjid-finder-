...
export async function POST(request: NextRequest) {
  ...
  const redirectUrl = contactMethod === "email" && destination ? `mailto:${destination}?subject=${encodeURIComponent("MasjidFinder plan enquiry")}` : contactMethod === "whatsapp" && destination ? `https://wa.me/${String(destination).replace(/[^0-9]/g, "")}?text=${encodeURIComponent("I have sent a MasjidFinder plan enquiry.")}` : null;
